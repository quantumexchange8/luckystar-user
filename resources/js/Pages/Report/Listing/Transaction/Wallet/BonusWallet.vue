...
const loadLazyData = (event) => {
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('transaction.getBonusWalletData', params);
            const response = await fetch(url);
    
            const results = await response.json();
            bonusWallet.value = results?.data?.data;
            totalRecords.value = results?.data?.total;
            bonusWalletCounts.value = results?.bonusWalletCounts;
        ...
        <template #content>
            <div class="w-full">
                ...
                >
                    ...
                    <template v-if="bonusWallet?.length > 0">
                        <Column
                            field="approval_at"
